...
Erfasst Video, schickt **jeden Frame durch eine XDNA1- oder XDNA2-NPU** und veröffentlicht das
Ergebnis an die virtuelle Kamera `/dev/video10` (nutzbar mit Zoom / Chrome / OBS / Meet).

> Die Aufnahme oben entstand auf XDNA1. Derselbe Quellcode nutzt nun die automatische
> Laufzeit-Geometrie und ein gerätespezifisches VMFB auch auf Strix Point `npu4`.

```
source ─▶ GStreamer appsink ─▶ NPU (2× 128×128 i32 matmul = 2D box blur) ─▶ appsrc ─▶ v4l2sink (/dev/video10)
```

Ursprüngliche XDNA1-Messung: **30 fps** mit 2 NPU-Dispatches/Frame, über
...
Die npu4-Verarbeitungsfunktion ist auf Hardware korrektheitsgeprüft; eine
XDNA2-Kamera-FPS wird noch nicht behauptet.
...
4. Ein Kernel für die aktuell installierte NPU:
   ```bash
   VMFB_OUT="$PWD/matmul.vmfb" ../../scripts/run-matmul.sh i32 128 128 128 2 3
   ```
   Das Skript erkennt XDNA1 (`npu1_4col`) oder Strix Point XDNA2 (`npu4`) und
   vergleicht vor dem Speichern jedes Ausgabeelement mit der CPU-Referenz.
