#!/usr/bin/env bash
# Build libnpu.so — the ctypes-loadable shared library (same as npu_runner but
# -fPIC -shared, C ABI, no main). g++ required.
set -euo pipefail
ROOT="${IREE_AMD_AIE_ROOT:-$HOME/src/iree-amd-aie}"
BLD="$ROOT/iree-build"
SRC="$ROOT/third_party/iree/runtime/src"; AMD="$ROOT/runtime/src"
GEN="$BLD/runtime/src"; AMDGEN="$BLD/runtime/plugins/AMD-AIE"
