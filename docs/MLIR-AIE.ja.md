...
この経路は **両方の NPU 世代** で検証済みですが、同一の wheel ではなく、
リリースごとの設計で検証しています:

> **XDNA1** — Lenovo ThinkPad T16 Gen2 · Ryzen 7 PRO 7840U（Phoenix, `npu1`）·
> Ubuntu 26.04 · kernel 7.0 · XRT 2.21 · 2026-06-24 に mlir-aie 1.3.x で検証
> （以前の単一 Worker 設計）。
>
> **XDNA2** — Ryzen AI 9 HX PRO 370（Strix Point, `npu2`, XRT 名
> `RyzenAI-npu4`）· Radeon 890M · Ubuntu 26.04 · kernel 7.0 · Ubuntu ネイティブの
> XRT 2.21.75 · NPU FW 1.1.2.64 · 2026-08-15 に **mlir-aie 1.4.1** で検証
> （現在のアノテーション付き単一 Worker 設計とアレイ全体設計）。

現在の 1.4.x 設計は XDNA1 では再検証していません。XDNA1 の項目は以前の
1.3.x の結果を記録したものです。
...
**ネイティブ bfp16ebs8** のエンドツーエンド設計（`ml/block_datatypes/…`）も
CPU の float 参照値と比較して実行し、スループットだけの測定では隠れる正しさの
限界を確認しました:

| ネイティブ bfp16 の形状（8 カラム） | スループット | CPU 参照結果 |
|---|---:|---|
| 512³ | 1.525 TFLOPS | **PASS** |
| 1024³ | 4.892 TFLOPS | **PASS** |
| 2048³ | 約 5.09 TFLOPS | **FAIL** — 1000 サンプル中 291、最大相対誤差 12% |

スループット値は、環境を v1.4.1 のピンに合わせる前の Peano 22
（`4a1adefa`）で記録しました。その後、固定 Peano 21（`c9c5ecb7`）で
PASS/FAIL スイープ全体を再実行し、境界が変わらないことを確認しました。
スクリプトは時間ではなく正しさだけをアサートします。

M=N=1024 のままリダクション長だけを分離すると、K=1216 は **PASS**、K=1280 は
**FAIL** です。[`check-bfp16-correctness.sh`](../scripts/check-bfp16-correctness.sh)
はこの既知の境界を再現してアサートします。ソースを見る限り、K タイルごとに
bfp16 出力を再ロードして保存し、部分和を繰り返し量子化している可能性があります。
これは K 依存性を説明する仮説であり、実証済みの修正ではありません。CPU 参照検査も
通らないネイティブ bfp のスループットは報告すべきではありません。これは上の表の
**bf16 入出力、内部 bfp16** の 2048³ 経路とは別であり、そちらの **4.64 TFLOPS**
は正しさの検査に合格しています。
