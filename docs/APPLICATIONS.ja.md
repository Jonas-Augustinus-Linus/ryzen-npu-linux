...
7840U で実測した。**CPU デノイザの 1 フレーム全体 (8 レイヤー) = 0.063 ms** に対し、
**NPU の 1 ディスパッチ = 3.8 ms** であり、CPU のフレーム全体より **約 60 倍遅い**。
単純に各レイヤーを 1 ディスパッチに割り当てる 8 レイヤー構成なら、1 フレームあたり
**8 × 3.8 ms = 30.4 ms** となり、CPU のフレームより **約 480 倍遅く**、10 ms の
リアルタイムバジェットも超過する。音声フレームは
